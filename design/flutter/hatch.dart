// Shared 45-degree hatch used by the board (dark squares) and the memory bar ("learning").
// Lines run bottom-left to top-right ("/"), like the prototype.
// REFERENCE IMPLEMENTATION (not compiled by the author).
import 'dart:math' as math;
import 'package:flutter/widgets.dart';

/// Paints parallel "/" lines spaced [gap] apart (perpendicular distance), each [width] thick,
/// across [size]. Clip the canvas first if the hatch should only appear in a region.
void paintHatch(
  Canvas canvas,
  Size size, {
  ...
  final paint = Paint()
    ...
    ..isAntiAlias = true;
  const n = Offset(0.7071067811865476, 0.7071067811865476); // perpendicular to the lines
  const d = Offset(-0.7071067811865476, 0.7071067811865476); // along the lines ("/")
