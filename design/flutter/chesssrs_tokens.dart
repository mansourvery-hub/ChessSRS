// ChessSRS design tokens: REFERENCE IMPLEMENTATION (not compiled by the author).
// Mirrors tokens/tokens.json. Depends only on package:flutter/widgets.dart so it survives
// Flutter's Material/Cupertino decoupling. Run `dart analyze` and fix any drift.
...
class SrsColors {
  ...
  final Color page, ground, surface;
  final Color ink, ink2, ink3;
  final Color hairline, hairlineSoft, scrim;
  final Color squareLight, squareDark, hatch, halo;
  final Color accent, accentSoft, accentMid;

  bool get isDark => brightness == Brightness.dark;

  factory SrsColors.light([SrsAccent a = kSrsDefaultAccent]) {
  ...
  factory SrsColors.dark([SrsAccent a = kSrsDefaultAccent]) {
  ...
  static SrsColors forBrightness(Brightness b, SrsAccent a) =>
...
/// Provide once near the root (below WidgetsApp), fed by user settings (theme mode + accent).
...
abstract final class SrsMotion {
  ...
  /// Respect the platform reduce-motion flag: `MediaQuery.disableAnimationsOf(context)`.
...
abstract final class SrsLayout {
  /// Measured on the app area (use LayoutBuilder), not on the window.
  ...
  static const double coordGutter = 24; // wide only, outside coordinates
  ...
  static const double topbarPadLeftWide = 50; // aligns scope label with the board's left edge
  static const double minTouchTarget = 44;

  /// [content] = area below the top bar and inside safe areas.
  /// Wide:   board = min(contentH - 64, contentW - 480)   (64 = bottom pad 28 + coords 24 + gap 12)
  /// Narrow: board = min(contentW - 24, contentH - 222)   (222 = minimum height reserved for the side column)
...
// ---------------------------------------------------------------------------
// Typography (fonts: Instrument Sans, Newsreader; bundle locally, see assets/fonts/README.md)
// ---------------------------------------------------------------------------
abstract final class SrsText {
  ...
  static TextStyle _ui(double size, FontWeight w, Color c,
          {double em = 0, double? height, bool tab = false}) =>
      TextStyle(
        fontFamily: ui,
        fontSize: size,
        fontWeight: w,
  ...
  /// Notation line. [wideWidth] is the app-area width; wide size = clamp(23, 2.4% of width, 31).
  static double lineSize({required bool wide, double wideWidth = 0}) =>
      wide ? (wideWidth * 0.024).clamp(23.0, 31.0).toDouble() : 21;
  static TextStyle lineMove(double size, Color c) =>
      _ui(size, FontWeight.w600, c, em: -0.012, height: wideLineHeight(size), tab: true);
  static TextStyle lineNumber(double size, Color c) =>
      _ui(size, FontWeight.w400, c, em: -0.012, height: wideLineHeight(size), tab: true);
  static double wideLineHeight(double size) => size > 21 ? 1.3 : 1.36;

  static TextStyle answerMove(bool wide, Color c) =>
      _ui(wide ? 52 : 38, FontWeight.w600, c, em: -0.03, height: 1.1);
  static TextStyle answerHelp(bool wide, Color c) => _ui(wide ? 16 : 15, FontWeight.w400, c, height: 1.4);

  /// Study commentary: the only place the reading face is used. Cap the measure at ~46 characters.
  static TextStyle note(bool wide, Color c) => TextStyle(
        fontFamily: read,
        fontSize: wide ? 19 : 16.5,
        height: wide ? 1.6 : 1.55,
        color: c,
      );
